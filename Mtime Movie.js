              ...
              },
              {
                ...
                events: {
                  draw: function(view, ctx) {
                    ...
                    ctx.addQuadCurveToPoint(width / 2, 0, width, height / 2)
                    ...
                    ctx.setAlpha(1)
                    ctx.moveToPoint(0, height)
                    ctx.addQuadCurveToPoint(width / 2, 0, width, height)
                    ctx.fillPath()
                  }
                }
              },
...
function activeMenu(index) {
  ...
  if (dstViewId == viewId) {
    ...
    for (var i = 0; i < 3; i++) {
      $("menu").cell($indexPath(0, i)).views[0].views[0].tintColor = index == i ? $color("darkGray") : $color("lightGray")
    }
    $(viewId).hidden = true
...
function actionTutorial() {
  ...
  var text1 = "Tips\n- Tap menu button to quickly scroll to top.\n- Tap menu button to quickly focus the text field in the search view.\n- LongPress menu button to quickly toggle tab.\n"
  var text2 = "- Swipe left to Favorite a movie in the recent view.\n"
  var text3 = "- Swipe left to Check/Uncheck or Delete a favorited movie in the favorite view.\n"
  var text4 = "- Swipe left to Locate a favorited movie in the local search view."
  
  var para = $objc("NSMutableParagraphStyle").invoke("alloc.init")
  para.invoke("setParagraphSpacing", 5)
  para.invoke("setParagraphSpacingBefore", 10)
  para.invoke("setLineSpacing", 2)
  para.invoke("setAlignment", $align.justified)
  
  // Attribute for text1
  var string1 = $objc("NSMutableAttributedString").invoke("alloc").invoke("initWithString", text1)
  string1.invoke("addAttribute:value:range:", "NSFont", $font("bold", 26), $range(0, 4))
  string1.invoke("setAlignment:range:", $align.center, $range(0, 4))
  
  // Attribute for text2
  var string2 = $objc("NSMutableAttributedString").invoke("alloc").invoke("initWithString", text2)
  string2.invoke("addAttribute:value:range:", "NSFont", $font("bold", 18), $range(text2.indexOf("Favorite"), 8))
  
  // Attribute for text3
  var string3 = $objc("NSMutableAttributedString").invoke("alloc").invoke("initWithString", text3)
  string3.invoke("addAttribute:value:range:", "NSFont", $font("bold", 18), $range(text3.indexOf("Check"), 13))
  string3.invoke("addAttribute:value:range:", "NSFont", $font("bold", 18), $range(text3.indexOf("Delete"), 6))
  
  // Attribute for text4
  var string4 = $objc("NSMutableAttributedString").invoke("alloc").invoke("initWithString", text4)
  string4.invoke("addAttribute:value:range:", "NSFont", $font("bold", 18), $range(text4.indexOf("Locate"), 6))
  
  // Combine
  var string = string1
  string.invoke("appendAttributedString", string2)
  string.invoke("appendAttributedString", string3)
  string.invoke("appendAttributedString", string4)
  
  // Paragraph attribute
  string.invoke("addAttribute:value:range:", "NSParagraphStyle", para, $range(4, string.invoke("length") - 4))
  
  $("label").runtimeValue().invoke("setAttributedText", string)
}
